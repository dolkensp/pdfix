...
public sealed class PdfEditor
    {
        public void RewriteWithEdits(
            ...
            IReadOnlyCollection<int>? pathsToRemove,
            ...
            (double x, double y)? newEnd)
    {
        ...
        for (var pageNumber = 1; pageNumber <= source.NumberOfPages; pageNumber++)
        {
            ...
            var removalSet = pathsToRemove != null ? new HashSet<int>(pathsToRemove) : new HashSet<int>();
            if (targetPathId.HasValue)
            {
                removalSet.Add(targetPathId.Value);
            }

            var invalidRemoval = removalSet.FirstOrDefault(id => id < 0 || id >= paths.Count);
            var invalidRemovalFound = removalSet.Any(id => id < 0 || id >= paths.Count);
            if (invalidRemovalFound)
            {
                throw new ArgumentOutOfRangeException(nameof(pathsToRemove), $"Path index {invalidRemoval} does not exist on page {pageNumber}.");
            }

            if (removalSet.Count > 0)
            {
                RemovePaths(sharpPage, removalSet);
            }

            if (targetPathId is null)
            {
                continue;
            }

            if (targetPathId < 0 || targetPathId >= paths.Count)
            {
                throw new ArgumentOutOfRangeException(nameof(targetPathId), $"Path index {targetPathId} does not exist on page {pageNumber}.");
            }
    ...
    private static void RemovePaths(PdfPage page, IReadOnlySet<int> pathIndicesToRemove)
    {
        if (pathIndicesToRemove.Count == 0)
        {
            return;
        }
        ...
        foreach (var element in content)
        {
            if (element is COperator op)
            {
                ...
                if (trackingPath)
                {
                    ...
                    {
                        currentPathIndex++;
                        if (!pathIndicesToRemove.Contains(currentPathIndex))
                        {
